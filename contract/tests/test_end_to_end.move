...
module message_board_addr::test_end_to_end {
    ...
    fun test_end_to_end(sender: &signer) {
        ...
        message_board::post_message(sender, string::utf8(b"SUP Tribe"));

        let string_content = message_board::get_message_content();
        assert!(string_content == string::utf8(b"SUP Tribe"), 3);

        // Post again, should overwrite the old message
        message_board::post_message(sender, string::utf8(b"hello Supra"));

        let string_content = message_board::get_message_content();
        assert!(string_content == string::utf8(b"hello Supra"), 16);
    }
}
